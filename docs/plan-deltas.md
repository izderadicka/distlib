...
| P1-18 | §9.1 | `distlib members` lists the group | **`members` needs the node stopped; a running node logs membership on change instead** | Same exclusive lock. A running node holds the database, so nothing outside the process can read it, and there is no local control API yet to ask through — building one now would be a phase 4 API surface arriving three phases early for one read-only command. So `run` reports the membership on every change, which is how a live group is observed, and `members` serves the stopped-node case with an error that names the reason. A control API is the right fix and belongs with the UI that needs it. |
...
| P1-25 | §7.1 | "localhost by default" | **Loopback plus a bearer token, both required**, in `<data-dir>/api.token` at mode 0600, held in `secrecy::SecretString` and never logged. `[api] enabled` switches the listener off. | Whoever can call this makes the node propose as itself. Narrower than holding the node's key — nothing proposed escapes the group's rules, and every proposal is signed and attributed — but not nothing, and there is no TLS. Loopback alone would trust every process on the machine; the token alone would trust the network. Also: a second node on one host needs its own `[api] bind_addr`, exactly as it needs its own `[net] bind_addr_v4`. |
| P1-26 | §3 | — | **hyper, not reqwest, for the API client** | The client only ever speaks HTTP to 127.0.0.1. reqwest's `rustls` feature pulls `aws-lc-rs` — a C toolchain — to satisfy a TLS stack nothing here uses, and feature unification with iroh's copy makes the no-provider build panic on first request. hyper and hyper-util are already in the tree via axum, so this costs zero new packages. It lives in `distlib-api` beside the server, for the same reason `RaftClient` sits beside `RaftProtocol`. |
