...
| P1-22 | §4.2, §4.5 | one dedicated ALPN for the Raft RPC; `distlib/memberlog/0` for log distribution to followers | **`distlib/raft/0` carries openraft's RPCs and nothing else, and is refused to any peer that is not a current voter. Proposals moved to `distlib/memberlog/0`**, which every member may speak. | The two have different audiences, and one ALPN serving both meant a node that accepted proposals from any member was also serving `Vote` and `AppendEntries` to them — so any member in the allowlist could disrupt a term. Splitting them makes the audience the protocol boundary. A node whose voter set is empty accepts from any member, which is the founding window and is bounded by the allowlist: without that exception a founder's `Vote` would be refused by peers who have not yet received the founding entry, and no group could ever be founded. |
