...
| P1-16 | §3, §4.3 | `net.allowlist` configures who a node talks to | **`net.allowlist` is gone; `[consensus] core` replaces it** | The allowlist is now derived from the log, so configuring it would be configuring a cache. What configuration still has to supply is the *founding* core group, and only until a group exists: founders cannot replicate the first entry without reaching each other, and cannot reach each other without an allowlist. So `[consensus] core` is read once, before `GroupFounded` applies, and never again. Unlike anything else that names a member it carries addresses, because the log that would otherwise supply them is exactly what those addresses are needed to fetch. `Config::load` rejects `net.allowlist` by name rather than letting `deny_unknown_fields` produce an error that does not say where the key went — every config file written before this release has one. |
