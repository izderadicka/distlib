...
- **openraft's `testing::Suite` has not run yet.** The plan put it in the log-store PR, but
  `Suite::test_all` exercises a `RaftLogStorage` *and* a `RaftStateMachine` together, and the state
  machine does not exist until the next PR. It runs there, covering the log store retroactively.
  The hand-written storage tests are not a substitute for it and were never meant to be.
