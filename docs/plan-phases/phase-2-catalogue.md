...
- **Phase 3 (API + UI)** — **C5**, because a UI is where "why can I not reach that member" gets
  asked; **C6**, to *measure* rather than to fix, since it is what a search will surface and the
  counter is an afternoon; **C8**, because "revisit when a phase wants to show who added something"
  is a description of phase 3; and **C12**, which is a test to write and needs no phase at all but
  may as well be done while the promotion code is still fresh.
...
- **No phase, and deliberately so** — **C1**, **C3**, **C7** and **C13**. The first three need a
  group or a catalogue bigger than anything that can be stood up today, and the fourth is
  unreachable by anything the code can currently produce. Assigning them to a phase would be
  scheduling work whose trigger is evidence, not a date; they stay here until something measures.
...
| **C6** | **A field blinks out of the read model while its newest value is in flight.** An entry arrives before its bytes do, so an item whose newest `title` entry is here and whose content is not reads with no title — and the projection writes `NULL` over a title that was searchable a moment ago, until the bytes land and `ContentReady` makes it re-project. | **Open, and open deliberately rather than unnoticed.** The obvious fix — keep the previous column value when the newest entry is unreadable — was rejected and must stay rejected while 2a-3's acceptance stands: it makes the tables a function of *what this node happened to see*. **Asked again after the phase, and the reason is sharper than the restart it was first written against.** The rows today are a function of (document, content this node holds) — no history — which is why a node that restarts re-derives the same rows. Keeping the previous value adds history as a third input, and the previous value is the one input that cannot be recovered from either of the other two: `read_item` queries `single_latest_per_key`, so the superseded entry is not what the projection reads, and where the same author overwrote it, it is not in the document at all. **The case that makes it plainly wrong is not a restart but a fresh join**, which needs no arranging: a node joining the group replays the document from scratch, never saw the superseded value, and writes `NULL`, while every older member shows a title the group has already replaced — permanently, if that content never arrives, which is exactly the content the repair sweep cannot fetch. `upsert_item` is `ON CONFLICT DO UPDATE SET` over every column, so "do not overwrite with `NULL`" is where the fix would go, and skipping the write entirely instead has the same defect by the same route. **One variant does survive the objection** and is worth writing down: project the newest *readable* entry per key rather than the newest entry. That keeps the inputs at (document, local content) with no history, so it cannot diverge the way the above does — and it helps only where the superseding write came from a *different* member, since iroh-docs keeps one entry per author and a member overwriting their own leaves nothing to fall back to. Nobody has measured how often the blink is the same-author case. What would close it properly is projecting the item only when every entry under it is readable, which trades this for a worse one: a single stuck `description` would hide a downloadable book. Nobody has measured how long the gap actually is on a real group — metadata values are under a kilobyte and usually arrive with the entry — so the size of the problem is still an argument rather than a number. Revisit when somebody sees an item vanish from a search. See P2-19. |
