...
- **2a-3 — `distlib-store`.** *(split while building it — see the note below.)* SQLite schema
...
| **Nothing asks again when a member cannot be resolved.** 2a-5 specifies asking the core group "whenever a member cannot be resolved" and only the startup half was built. | **Open.** There is no single place that detects it: a dial failure surfaces inside iroh, inside iroh-docs' downloader, and at each protocol client separately, and the three do not agree on what "cannot be resolved" means — one has no address at all, another has a stale one. Wiring them together is its own piece of work, and doing it badly means a node that answers every transient failure with an RPC to a core node. What is wanted first is a single point where "we had no address for this member" is observable; the ask is one line once that exists. |
