...
| **Content nobody will serve is asked for for ever.** The sweep that repairs a stranded entry (P2-16) has no backoff: a node holding an entry whose bytes no reachable member has — an expelled one, most plainly — dials the group for it every five seconds for as long as it runs. | **Open.** The cost is bounded and small: one round of dials on a timer, and nothing at all on a node whose content is complete, since a hash whose bytes are present is never asked for. What the backoff should be is the part not being guessed at — it wants a measurement of how often this state is reached and how long it lasts, and nobody has watched a group long enough to have one. Revisit with the O(N²) offer above, which is the same shape of question. |
